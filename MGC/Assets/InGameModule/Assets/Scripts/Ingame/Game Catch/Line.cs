...
public class Line : MonoBehaviour
{
    ...
    // 안에서 밖으로 나갈 때 교점 연산
    public void SetIntersectionPoint(Vector2 _vAP1, Vector2 _vAP2, Vector2 _vBP1, int _iIdx)
    {
        ...
        if(Math.GetIntersectPoint(_vAP1, _vAP2, _vBP1, vBP2, ref vIntersectionPoint))
        {
            if (m_listPoints == null)
            {
                ...
                SendPoint(vIntersectionPoint, _iIdx);
    ...
    // 밖에서 안으로 들어올 때 교점 연산
    public void SetIntersectionPoint(Vector2 _vAP1, Vector2 _vAP2, Vector2 _vBP1, Vector2 _vBP2, int _iIdx)
    {
        Vector2 vIntersectionPoint = new Vector2();
    ...
    public void UpdateLine(Vector2 _vMousePos, int _iIdx)
    {
        //1// 첫 점이면 거리 계산 없이 찍음
        if (m_listPoints == null)
        {
            m_listPoints = new List<Vector2>();
            SendPoint(_vMousePos, _iIdx);
            SetPoint(_vMousePos);
    ...
    private void SendPoint(Vector2 _vPoint, int _iIdx)
    {
        GameData.SendPointPacket point = new GameData.SendPointPacket();
        point.iGameType = (int)GameData.EnumGameType.CATCH;
        point.iStructType = (int)GameData.EnumGameCatchStructType.SEND_POINT_PACKET;
        point.iColor = 0;
